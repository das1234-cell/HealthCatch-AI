...
// SIMPLIFIED 3D - Removed complex DNA helix to prevent rendering issues

// If you want the full 3D back, use App_3D_VERSION.jsx
...
function DNAHelix({ scrollProgress }) {
  ...
  return null; // Simplified - no DNA rendering

}



// DNA components disabled for performance

function DNASphere() { return null; }

function DNARung() { return null; }
...
/* ═══════════════════════════════════════════════════════════════
   §5B  HEALTHCATCH FINAL HEALTH COMMAND CENTER
   Additive feature layer. Existing legacy components remain intact.
   ═══════════════════════════════════════════════════════════════ */

function HealthCommandCenterPanel({ panelBg, panelBorder, textPrimary, textSecondary }) {
  ...
  const timeline = [
    ['18 Aug 2026', 'Blood report uploaded', 'Medical Vault'],
    ['17 Aug 2026', 'Completed 10K steps', '+120 XP'],
    ['15 Aug 2026', 'Prescription updated', 'Dr. Amit Patel'],
    ['12 Aug 2026', 'Joined Mental Wellness', 'Community'],
  ];

  const cards = [
    { title:'Health Timeline', text:'Your wellness events, reports, consultations and achievements in one chronological view.', icon:<Clock3 size={20}/>, accent:'#60a5fa' },
    { title:'Medication Manager', text:'Track medicines, schedules, adherence and refill reminders without changing prescriptions.', icon:<Pill size={20}/>, accent:'#34d399' },
    { title:'Lab Report Intelligence', text:'Organize uploaded reports and compare trends across dates before talking to a professional.', icon:<FolderHeart size={20}/>, accent:'#a78bfa' },
    { title:'Family Health Vault', text:'Separate family profiles with controlled sharing for records, appointments and emergency information.', icon:<UsersRound size={20}/>, accent:'#f59e0b' },
    { title:'Device Sync', text:'Connect health data from supported devices in a future Firebase/Health Connect integration.', icon:<Activity size={20}/>, accent:'#22d3ee' },
    { title:'Nutrition Scanner', text:'Demo food-analysis workflow for calories and macro estimates from an image.', icon:<Utensils size={20}/>, accent:'#f472b6' },
    { title:'Emergency Card', text:'A compact emergency view for blood group, allergies, trusted contacts and care actions.', icon:<HeartHandshake size={20}/>, accent:'#ef4444' },
    { title:'Rewards & XP', text:'Turn healthy habits into streaks, levels, badges and community rewards.', icon:<Award size={20}/>, accent:'#fbbf24' },
  ];
  ...
  return (
    <motion.div
      ...
      animate="animate"
      exit="exit"
      style={{height:'100%',overflowY:'auto',position:'absolute',inset:0,padding:'38px 36px 120px'}}
    >
      <div style={{maxWidth:1120,margin:'0 auto'}}>
        <div style={{display:'flex',alignItems:'center',gap:9,color:'#60a5fa',fontSize:11,fontWeight:800,letterSpacing:'.14em',textTransform:'uppercase',marginBottom:10}}>
          <CircleGauge size={15}/> Health Command Center
        </div>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'end',gap:20,flexWrap:'wrap',marginBottom:24}}>
          <div>
            <h2 style={{fontSize:'clamp(28px,4vw,40px)',fontWeight:900,fontFamily:"'Space Grotesk',sans-serif"}}>Your complete health workspace.</h2>
            <p style={{maxWidth:780,color:textSecondary,fontSize:13.5,lineHeight:1.7,marginTop:9}}>
              One place for your health timeline, medicines, reports, devices, nutrition, family records, emergency details and rewards. The UI is production-oriented; real clinical data connections should be added behind authenticated Firebase rules.
            </p>
          </div>
          <div style={{padding:'9px 12px',borderRadius:12,background:'rgba(16,185,129,.08)',border:'1px solid rgba(16,185,129,.16)',color:'#34d399',fontSize:10,fontWeight:800}}>HEALTH OS • ACTIVE</div>
        </div>

        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:15,marginBottom:20}}>
          {cards.map(card => (
            <div key={card.title} style={{background:panelBg,border:`1px solid ${panelBorder}`,borderRadius:20,padding:20,backdropFilter:'blur(35px)',minHeight:166,display:'flex',flexDirection:'column'}}>
              <div style={{width:42,height:42,borderRadius:13,display:'grid',placeItems:'center',background:`${card.accent}14`,color:card.accent,marginBottom:15}}>{card.icon}</div>
              <h3 style={{fontSize:15,fontWeight:900}}>{card.title}</h3>
              <p style={{fontSize:11.5,color:textSecondary,lineHeight:1.55,marginTop:7,flex:1}}>{card.text}</p>
            </div>
          ))}
        </div>

        <div style={{display:'grid',gridTemplateColumns:'1.15fr .85fr',gap:18,alignItems:'start'}}>
          <div style={{background:panelBg,border:`1px solid ${panelBorder}`,borderRadius:22,padding:22,backdropFilter:'blur(35px)'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:15}}>
              <div><p style={{fontSize:10,color:'#60a5fa',fontWeight:800,letterSpacing:'.1em',textTransform:'uppercase'}}>Health Timeline</p><h3 style={{fontSize:19,fontWeight:900,marginTop:4}}>Recent health activity</h3></div>
              <Activity size={18} color="#60a5fa"/>
            </div>
            {timeline.map(([date,title,meta],i)=>(
              <div key={title} style={{display:'grid',gridTemplateColumns:'92px 1fr auto',gap:12,alignItems:'center',padding:'14px 0',borderTop:i===0?'none':`1px solid ${panelBorder}`}}>
                <span style={{fontSize:10.5,color:textSecondary}}>{date}</span>
                <div><p style={{fontSize:12.5,fontWeight:800}}>{title}</p><p style={{fontSize:10.5,color:textSecondary,marginTop:3}}>{meta}</p></div>
                <ArrowUpRight size={15} color="#60a5fa"/>
              </div>
            ))}
          </div>

          <div style={{background:panelBg,border:`1px solid ${panelBorder}`,borderRadius:22,padding:22,backdropFilter:'blur(35px)'}}>
            <div style={{display:'flex',alignItems:'center',gap:9,marginBottom:15}}><ShieldCheck size={18} color="#34d399"/><h3 style={{fontSize:18,fontWeight:900}}>Quick Actions</h3></div>
            <button onClick={()=>setMedChecked(v=>!v)} style={{width:'100%',padding:13,borderRadius:12,border:`1px solid ${panelBorder}`,background:'transparent',color:textPrimary,cursor:'pointer',textAlign:'left',marginBottom:9}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}><span style={{fontSize:12,fontWeight:800}}>Medication check-in</span><span style={{fontSize:10,color:medChecked?'#34d399':textSecondary,fontWeight:800}}>{medChecked?'DONE':'MARK DONE'}</span></div><p style={{fontSize:10.5,color:textSecondary,marginTop:5}}>Today · 8:00 PM</p></button>
            <button onClick={()=>setDeviceConnected(v=>!v)} style={{width:'100%',padding:13,borderRadius:12,border:`1px solid ${panelBorder}`,background:'transparent',color:textPrimary,cursor:'pointer',textAlign:'left',marginBottom:9}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}><span style={{fontSize:12,fontWeight:800}}>Wearable device</span><span style={{fontSize:10,color:deviceConnected?'#34d399':textSecondary,fontWeight:800}}>{deviceConnected?'CONNECTED':'CONNECT'}</span></div><p style={{fontSize:10.5,color:textSecondary,marginTop:5}}>Steps, sleep and activity sync</p></button>
            <button onClick={()=>setNutritionScanned(v=>!v)} style={{width:'100%',padding:13,borderRadius:12,border:`1px solid ${panelBorder}`,background:'transparent',color:textPrimary,cursor:'pointer',textAlign:'left',marginBottom:9}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}><span style={{fontSize:12,fontWeight:800}}>Nutrition scanner</span><span style={{fontSize:10,color:nutritionScanned?'#34d399':textSecondary,fontWeight:800}}>{nutritionScanned?'PREVIEW READY':'SCAN FOOD'}</span></div><p style={{fontSize:10.5,color:textSecondary,marginTop:5}}>{nutritionScanned?'Demo estimate: ~420 kcal · 24g protein':'Upload a food image in the next version'}</p></button>
            <button onClick={()=>setFamilySharing(v=>!v)} style={{width:'100%',padding:13,borderRadius:12,border:`1px solid ${panelBorder}`,background:'transparent',color:textPrimary,cursor:'pointer',textAlign:'left'}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}><span style={{fontSize:12,fontWeight:800}}>Family vault sharing</span><span style={{fontSize:10,color:familySharing?'#34d399':textSecondary,fontWeight:800}}>{familySharing?'ENABLED':'PRIVATE'}</span></div><p style={{fontSize:10.5,color:textSecondary,marginTop:5}}>{familySharing?'Demo: trusted contact can view selected records':'No family member has access yet'}</p></button>
          </div>
        </div>

        <div style={{marginTop:18,display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(240px,1fr))',gap:15}}>
          <div style={{padding:18,borderRadius:18,background:'rgba(239,68,68,.07)',border:'1px solid rgba(239,68,68,.16)'}}><div style={{display:'flex',alignItems:'center',gap:9,color:'#f87171',marginBottom:8}}><Phone size={17}/><strong style={{fontSize:13}}>Emergency Card</strong></div><p style={{fontSize:11,color:textSecondary,lineHeight:1.55}}>Keep critical information ready: blood group, allergies, trusted contact and care preferences.</p></div>
          <div style={{padding:18,borderRadius:18,background:'rgba(245,158,11,.07)',border:'1px solid rgba(245,158,11,.15)'}}><div style={{display:'flex',alignItems:'center',gap:9,color:'#fbbf24',marginBottom:8}}><Award size={17}/><strong style={{fontSize:13}}>Rewards & XP</strong></div><p style={{fontSize:11,color:textSecondary,lineHeight:1.55}}>12 badges unlocked · 1,240 XP · 9-day streak. Rewards should come from verified activity.</p></div>
          <div style={{padding:18,borderRadius:18,background:'rgba(59,130,246,.07)',border:'1px solid rgba(59,130,246,.15)'}}><div style={{display:'flex',alignItems:'center',gap:9,color:'#60a5fa',marginBottom:8}}><ShieldCheck size={17}/><strong style={{fontSize:13}}>Consent Center</strong></div><p style={{fontSize:11,color:textSecondary,lineHeight:1.55}}>Control what the AI, professionals, family members and communities can access.</p></div>
        </div>
      </div>
    </motion.div>
  );
      ...
      <motion.div initial={{ x: -280 }} animate={{ x: 0 }} transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }} style={{ width: '256px', display: 'flex', flexDirection: 'column', background: panelBg, backdropFilter: 'blur(50px)', borderRight: `1px solid ${panelBorder}` }}>
        ...
        <div style={{ padding: '28px 22px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
            ...
            <SidebarBtn icon={<CircleGauge size={17} />} label="Health Center" tab="health-center" active={activeTab} set={setActiveTab} isDark={isDarkMode} />
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100%' }}>
                  ...
                  <motion.div initial={{ opacity: 0, y: 8, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 8, scale: 0.96 }} style={{ position: 'absolute', right: 0, top: '48px', width: '320px', borderRadius: '16px', background: isDarkMode ? 'rgba(15,23,42,0.92)' : 'rgba(255,255,255,0.95)', border: `1px solid ${panelBorder}`, backdropFilter: 'blur(40px)', zIndex: 100, overflow: 'hidden', boxShadow: '0 20px 60px -15px rgba(0,0,0,0.6)' }}>
                    ...
                    <div style={{ padding: '12px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                      ...
                      {[
                        ...
                        { icon: <CircleGauge size={20}/>, label: 'Health Center', tab: 'health-center' }
        ...
        <div style={{ flex: 1, position: 'relative', overflow: 'hidden' }}>
          ...
          <AnimatePresence mode="wait">
            ...
            {/* FINAL HEALTH COMMAND CENTER — additive only */}

            {activeTab === 'health-center' && (

              <HealthCommandCenterPanel
                panelBg={panelBg}
...
function App() {
  ...
  // Keep login state in sync with Firebase's actual persisted session,

  // so a page refresh doesn't kick the user back to the login screen.
  ...
  if (!isAuthReady) {
    ...
    return (
      ...
      <div style={{
        ...
        background: isDarkMode ? '#000000' : '#f8fafc', color: isDarkMode ? '#ffffff' : '#0f172a',
