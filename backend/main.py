...
async def chat_with_agent(message: str = Form(""), file: UploadFile = File(None)):
    ...
    try:
        ...
        # API-তে কল করা
        # IMPORTANT: `requests.post` is a BLOCKING call. Calling it directly inside an
        # `async def` endpoint freezes FastAPI's entire single-threaded event loop for
        # as long as the request takes (this free model can take many seconds) — that
        # means EVERY other user/request on the whole backend stalls too, which is
        # what made the AI chat (and anything else hitting this server) feel laggy.
        # `asyncio.to_thread` runs the blocking call in a worker thread instead, so
        # the event loop stays free to handle other requests while we wait.
        response = await asyncio.to_thread(
            requests.post,
            "https://openrouter.ai/api/v1/chat/completions",
            headers=headers,
            json=data,
            timeout=30,  # fail fast instead of hanging forever if OpenRouter stalls
        )
        ...
        return {"reply": "The AI is taking too long to respond right now. Please try again in a moment."}
